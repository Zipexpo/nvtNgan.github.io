...
//d3.json("data/karate.json", function(error, graph) {
	d3.json("data/lesmis.json", function(error, graph) {
...
//end
  ...
  node.append("title")
      .text(function(d){return takeaname(d);});
  node.append("text")
    .attr("dx", 12)
    .attr("dy", ".35em")
    .text(function(d){return takeaname(d);});
  
  simulation.force("charge", d3.forceManyBody().strength(function(){return -wgroup/graph.nodes.length*5}));
  simulation
      .nodes(graph.nodes)
      .on("tick", ticked);
	...
	function update_group(depth){
		var cg = 0;
		  ...
		  for (var j=0;j<leave.length;j++){
			...
			node2.data(leave[j].leaves(), function(d){return takeaname(d.data);})
			.selectAll("circle")
			.attr("fill",function(d) { return color(j); });
			var node_temp=[];
			link2.data(leave[j].descendants(), function(d){return takeaname(d.data);})
